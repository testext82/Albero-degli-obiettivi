...
- **Sincronizzazione con Supabase** (facoltativa): account con email e password, dati
  protetti da Row Level Security e aggiornamenti in tempo reale su tutti i dispositivi.
...
**5. Disattiva la conferma via email.** In *Authentication → Sign In / Providers → Email*
disattiva **Confirm email** (o "Enable email confirmations") e salva. Così registrazione
e accesso avvengono con email + password **senza inviare alcuna email**: nessun link,
nessun codice, nessun servizio SMTP da configurare, e funziona anche con l'app installata
su iPhone.

**6. Usa l'app.** Apri l'app, premi **Sincronizza**. La prima volta premi **Registrati**
con email e una password a scelta (minimo 6 caratteri): entri subito. Le volte successive
usi **Accedi**; il dispositivo resta collegato.
